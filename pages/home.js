...
export default function Home() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setSnackbarMessage("OCR processed successfully!");
    } catch (error) {
      console.error("OCR error:", error);
      setSnackbarMessage("OCR process failed.");
    } finally {
  ...
  return (
    <Box
      textAlign="center"
      p={3}
      style={{ maxWidth: "600px", margin: "auto", backgroundColor: "#f9f9f9" }}
    >
      ...
      {/* OCR Result */}
      {ocrText && (
        <Box mt={4} p={2} border="1px solid #ddd" borderRadius="8px">
